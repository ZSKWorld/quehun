(function (exports, Laya) {
    ...
    class TtVideoTexture extends Laya.VideoTexture {
        constructor() {
            super();
            this._volume = 1;
            this._muted = false;
            this._ended = false;
            this.decoder = Laya.PAL.g.createOffscreenVideo();
            this.decoder.onCandraw((aspectRatio) => {
                if (this._loaded)
                    return;
        ...
        onLoad(url) {
            ...
            if (this._source !== src)
                return;
            this.decoder.src = Laya.URL.postFormatURL(Laya.URL.formatURL(url));
        ...
        onRender() {
            Laya.LayaGL.textureContext.updateVideoTexture(this._texture, this.decoder, false, false);
            return true;
        }
        onDestroy() {
    ...
    MgBrowserAdapter.beforeInit = function () {
        Laya.Browser.onTTMiniGame = true;
        ...
        Laya.PAL.g = window.tt;
    };
    MgBrowserAdapter.afterInit = function () {
        Laya.PAL.media.shortAudioClass = Laya.WebAudioChannel;
        if (Laya.Browser.onDevTools) {
            Laya.PAL.media.videoTextureClass = null;
            Laya.PAL.media.videoPlayerClass = null;
        }
        else {
            Laya.PAL.media.videoTextureClass = TtVideoTexture;
            Laya.PAL.media.videoPlayerClass = MgVideoPlayer;
        }
    };
    ...
    exports.TtVideoTexture = TtVideoTexture;
