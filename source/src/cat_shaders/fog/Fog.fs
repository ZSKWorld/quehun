...
void main()
{
    ...
    vec2 c = texture2D(u_DecorativeTex, vec2(iuv.x, iuv.y + u_Time * 0.05 * (0.3 - u_ScaleController * 0.15))).gb + texture2D(u_DecorativeTex, vec2(iuv.x + u_Time * 0.05 * 0.2, iuv.y)).gb - vec2(1.0, 1.0);
    ...
    h = mix(mix(h, h2, u_ScaleController * 0.5), h3, (max(0.0, u_ScaleController - 1.0)));
    h.a = ahlpaMask.r * (1.0 - u_AlphaController) * (fresnelFactor* 3.0);
    h.a = mix(h.a, h.a * (1.0 - max(0.0,(h.b - h.r))), u_CloudController);
    gl_FragColor = h;
}
