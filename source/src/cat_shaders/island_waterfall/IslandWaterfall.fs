...
void main()
{
    ...
    vec2 node_373 = v_uv0;
    vec4 _waterfall_var = texture2D(u_waterfall,TransformUV(node_373, u_to4));
    vec3 node_7741 = (((u_bright*(u_waterColor.rgb*_MainTex_var.rgb))+clamp((_wave_var.r*(1.0-node_1592)),0.0,1.0))+_waterfall_var.rgb);

    vec4 _AO_var = texture2D(u_AO,TransformUV(v_uv1, u_to5));
    vec3 emissive = (lerp(node_7741,u_lightColor.rgb,_AO_var.r)*(node_7741*lerp(u_aoColor.rgb,vec3(u_aoStrenth,u_aoStrenth,u_aoStrenth),_AO_var.r)));
