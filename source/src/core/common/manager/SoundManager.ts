...
class AudioInfo {
    ...
    onFinished() {
        ...
        SoundManager.StopAudio(this.audio_id);
...
export class SoundManager {
    ...
    static PlayAudioByCustomInfo(obj) {
        if (obj.data || obj.parent.data) {
            ...
            for (let i: number = 0; i < list_d.length; i++) {
                ...
                if (v[0] == 'audio') {
                    SoundManager.PlayAudio(Number(v[1]));
                    return;
                }
            }
        }
    ...
    static onMusicChange(tween_time: number = 1000) {
        ...
        //音乐状态改变时分发监听事件
        for (let i = 0; i < SoundManager.voiceChangeHandles.length; i++) {
            SoundManager.voiceChangeHandles[i] && SoundManager.voiceChangeHandles[i].run();
        }
    }

    static registMusicChangeHandle(onMusicChangeHandle: Laya.Handler) {
        SoundManager.voiceChangeHandles.push(onMusicChangeHandle);
    }

    static unRegistMusicChangeHandle(onMusicChangeHandle: Laya.Handler) {
        for (let i = 0; i < SoundManager.voiceChangeHandles.length; i++) {
            if (SoundManager.voiceChangeHandles[i] == onMusicChangeHandle) {
                SoundManager.voiceChangeHandles.splice(i, 1);
                break;
            }
        }
    }
