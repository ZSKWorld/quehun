...
export class RenderChooseBattleView extends ExtensionClass<IView, RenderChooseBattle>(RenderChooseBattle) implements IView {

    override onCreate() {
        const { graph_touch, btn_break } = this;
        graph_touch.onClick(this, this.sendEvent, [RenderChooseBattleMsg.OnGraphTouchClick]);
        btn_break.onClick(this, this.sendEvent, [RenderChooseBattleMsg.OnBtnBreakClick]);
    }
    ...
    refreshBossCool(data: CfgBossData) {
        ...
        txt_content5.text = isEnd ? "" : TimeUtil.timeFormat(coolTime);
    }
    ...
    refreshGatherCool(data: CfgGatherData) {
        ...
        !isEnd && (txt_content4.text = `采集中：${ TimeUtil.timeFormat(lastTime) }`);
