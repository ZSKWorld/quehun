...
export class RenderGetRewardItemView extends ExtensionClass<IView, RenderGetRewardItem>(RenderGetRewardItem) implements IView {

	refresh(data: IRewardSlot) {
		this.visible = !!data;
		if (!data) return;
		this.com_item.refresh(data.id);
