...
import { ComItem2048Mediator } from "./coms/ComItem2048Mediator";
...
class Chessboard2048 extends Observer {
	...
	addRandom(addCnt: number) {
		...
		for (let i = 0; i < addCnt; i++) {
			...
			const cell = ComItem2048Mediator.create(value, rowArr[addIndex], colArr[addIndex]);
	...
	clear() {
		this._moving = false;
		this._tempCells.forEach(v => ComItem2048Mediator.recover(v));
		...
		this._cells.forEach(v => v.forEach((v1, i) => {
			ComItem2048Mediator.recover(v1);
			v[i] = null;
		}));
	}
	...
	private addAt(row: number, col: number, value: number) {
		...
		const cell = ComItem2048Mediator.create(value, row, col);
		cell.view.setXY(bg.x, bg.y);
		_container.addChild(cell.view);
		_cells[row][col] = cell;
	}
	...
	private getCellBg() {
		const bg = Laya.Pool.getItemByCreateFun("Cell2048Bg", () => {
			const result = <fgui.GImage>fgui.UIPackage.createObjectFromURL("ui://vx9zwserqjdo2c");
...
export class UI2048Mediator extends MediatorBase<UI2048View, UI2048Data> {
	private _chessboard: Chessboard2048;

	override onAwake() {
		this.addEvent(UI2048Msg.OnBtnBackClick, this.onBtnBackClick);
		this.addEvent(UI2048Msg.OnBtnResetClick, this.onBtnResetClick);
	...
	private onBtnBackClick() {
		this.openView(ViewID.UILittleGameView);
		this.closeSelf();
	...
	@InterestNotify(ChessboardEvent.GameOver)
	private onGameOver() {
		$confirm("游戏结束", "游戏结束");
	}
}