...
export class ComItem2048Mediator extends MediatorBase<ComItem2048View, ComItem2048Data> {
    ...
    static create(value: number, row: number, col: number) {
        const item = Laya.Pool.getItemByCreateFun("ComItem2048Mediator", () => {
            const result = <ComItem2048Mediator>facade.createMediator(ViewID.ComItem2048View);
    ...
    static recover(item: ComItem2048Mediator) {
        if (!item) return;
        item.value = 0;
        item._mergeValue = 0;
        item.setPos(-1, -1);
        ...
        Laya.Pool.recover("ComItem2048Mediator", item);
    }
    ...
    private onMoveCompleted() {
        ...
        if (mergeValue > 0) {
            ...
            ComItem2048Mediator.recover(this);
        }
    }
}