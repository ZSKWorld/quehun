...
class GoodsBase<T> extends DecodeObject<T> {
    id: number;
    ...
    get useRequireStr() { return UserUtil.getLevelStr(this.useRequire); }
}
...
export class Equipment extends GoodsBase<IEquipment> implements IEquipment {
    ...
    get infoStr() {
        ...
        return [
            `类别:${ typeStr[this.part] }`,
            `境界需求:${ this.useRequireStr }`,
            `铭刻:0/100`,
            `${ this.shenYou }阶神佑加成: ${ (/**equip.shenYouAddition */0 * 100).toFixed(2) }%`,
            `${ this.mainAttri.map(v => GameUtil.getColorStr(this.quality, UserUtil.getAttributeName(v))).join("<br/>") }`,
            `${ this.secondAttri.map(v => GameUtil.getColorStr(5, UserUtil.getAttributeName(v))).join("<br/>") }`,
            `${ this.bodyAttri.map(v => GameUtil.getColorStr(8, UserUtil.getAttributeName(v))).join("<br/>") }`,
        ].filter(v => !!v).join("<br/>");
    }
    get score() { return 0; }
}